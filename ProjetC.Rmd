...
M1 Economiste d'entreprise\\Projet Analyse de données exploratoire\\
    ...
    \HRule
    \vspace*{0.5cm}
    \large{Basma \textsc{Ghaffour}, Aybuké \textsc{Bicat}}\\
    \vspace{3.75cm}
...
On a choisi de travailler sur les bases de données\footnote{\href{https://github.com/jldbc/coffee-quality-database/find/master}{Sources des bases de données}} portant sur le café.  La problématique est la suivante : la qualité du café dépend-elle de sa provenance ? On a travaillé sur une base de données qui contient 1339 observations et 44 variables. Pour y répondre on a fait le choix de faire une analyse factorielle par composante principale dans un premier temps.
...
```
\textbf{\underline{Analyse descriptive:}} 
...
\textbf{\underline{Profils colonnes:}}
...
\textbf{\underline{Profils lignes:}}
...
Sur ce graphique, on peut voir les inerties relatives pour chacun des axes. On a fait le choix de garder les deux premiers axes qui cumulent environ **`r round(Int[3,2],0)`%**\footnote{On peut voir les données des inerties relatives cumulées sur la table 16 en annexe} de l'inertie totale.
...
```{r graphqiue des contributions pour laxe 1,fig.width=5,fig.height=3}
...
```{r graphique des contribution pour laxe 2,fig.width=6,fig.height=3}
...
```{r representation des modalite,fig.height=3.75,fig.width=6}
...
En faisant le test  du $\chi ^{2}$, on voit que les variables expliquent très bien la partition. On continue donc notre étude avec **3 clusters**.\footnote{On peut voir les resultat du test sur la table 17 en annexe}
...
\textbf{\underline{Parangons :}} 


```{r, echo=FALSE}

tabl(data_bis[labels(cah$desc.ind$para$'1'),],
     "Parangon du cluster 1")
```
...
```{r, echo=FALSE}
tabl(data_bis[labels(cah$desc.ind$para$'2'),],
      "Parangon du cluster 2")
...
tabl(data_bis[labels(cah$desc.ind$para$'3'),],
     "Parangon du cluster 3")
...
\textbf{\underline{Individus extrêmes :}}

```{r, echo=FALSE}

tabl(data_bis[labels(cah$desc.ind$dist$'1'),], 
     "Individus extrêmes du cluster 1 ")
...
tabl(data_bis[labels(cah$desc.ind$dist$'2'),],
     "Individus extrêmes du cluster 2 ")
...
tabl(data_bis[labels(cah$desc.ind$dist$'3'),],
     "Individus extrêmes du cluster 3")
...
```{r test}
tabl(cah$desc.var$test.chi2,"resultat du test du khi deux")
```

```{r}
tabl(B3,"proportion d'observation par modalité d'altitude")
```
